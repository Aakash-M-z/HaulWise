...
def trip_list_create(request):
    if request.method == 'POST':
        ...
        response_data = {
            ...
            'initialRemainingCycleHours': plan.get('initialRemainingCycleHours', max(0.0, 70.0 - cycle_used)),
            'remainingCycleHours': plan['remainingCycleHours'],
            'totalDutyHours': plan.get('totalDutyHours', 0.0),
            'isCycleInsufficient': plan.get('isCycleInsufficient', False),
            'cycleWarningMessage': plan.get('cycleWarningMessage', ''),
            'trip_plan': plan,
